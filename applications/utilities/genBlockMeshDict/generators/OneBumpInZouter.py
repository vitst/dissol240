...
class OneBumpInZouter(gbm.AbstractBaseGenerator):
  '''
  It is an asymmetric case where the bump is on one wall and directed outside of the fracture.
  '''

  def __init__(self):
    self.__genName__ = 'oneBumpOut'
    ...
    self.parameters.append(['n_spline',    500, 'number of points for the spline'])
  ...
  def bump_function(self, y, z):
    bump_z = self.get_parameter('bump_z')
    A = self.get_parameter('bump_A')
    wl = self.get_parameter('bump_w')

    newY = y

    if( z > bump_z-wl/2. and z < bump_z+wl/2.):
      rel_z = z - (bump_z-wl/2.)
  ...
  def createBlockMeshDict(self, dictFileName):
    ...
    if self.__remesh__:
      ...
      for i in range(n_spline):
        ...
        currentY =  Ly/2.
        currentZ = i*dz
        slp26.append([currentX, currentY, currentZ])
        currentX = -Lx/2.
        currentY =  Ly/2.
        currentZ = i*dz
        slp37.append([currentX, currentY, currentZ])
    ...
    Lbump = bump_w * 4.0
    bump_z1 = bump_z - Lbump/2.
    bump_z2 = bump_z + Lbump/2.
    ...
    total_n = n_bump + res_top + res_low

    perC1 = res_low / float(total_n)
    perC2 = n_bump / float(total_n)
    perC3 = res_top / float(total_n)
