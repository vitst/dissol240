...
)
{
    ...
    scalarList residual_magn;
    scalarList residual_norm;
    List<solverPerformance> solverPerfVector;
    ...
    for (direction cmpt=0; cmpt<Type::nComponents; cmpt++)
    {
        ...
        // Calc initial norm for this direction
        scalar solverDirNorm = lduMatrix::solver::New
        (
            ...
            *this,
            ...
            interfaces,
            solverControls
        )->normFactor(psiCmpt, sourceCmpt, cmpt);
        ...
        scalar gSumMag_rA_ = solverDirNorm * solverPerf.initialResidual();
        residual_magn.append(gSumMag_rA_);
        residual_norm.append(solverDirNorm);
        solverPerfVector.append( solverPerfVec );
    ...
    scalar vecNorm = 0;         // Calculate norm of size components
    for (int i = 0; i < residual_norm.size(); i++){
        vecNorm += residual_norm[i] * residual_norm[i];
    }
    vecNorm = sqrt(vecNorm);
    
    solverPerfVec.initialResidual() = 0.0;
    for (int i = 0; i < solverPerfVector.size(); i++){
        solverPerfVector[i].initialResidual() = residual_magn[i] / vecNorm;
      
        if (debug)
        {
            Info<< "Component " << i << ": initialResidual()   " 
                << solverPerfVector[i].initialResidual() << endl;
        }
      
        if (solverPerfVector[i].initialResidual() >
            solverPerfVec.initialResidual()
        && !solverPerfVector[i].singular())
        {
            solverPerfVec = solverPerfVector[i];
        }
    }
